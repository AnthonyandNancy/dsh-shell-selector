...
function resolveInPath(exe, env, extraDirs = []) {
...
function addCandidate(list, seen, path, source) {
...
function addGitRootCandidates(list, seen, root, source) {
...
export function bashCandidates(platform = process.platform, env = process.env) {
    ...
    const add = (path, source) => {
        if (!isExecutableFile(path) || isWslBash(path, env))
            return;
 ...
 */
export function probeBashSync(path, env = process.env) {
 ...
 */
export function bashAvailability(platform = process.platform, env = process.env) {
    if (platform !== 'win32') {
 ...
 */
export function pwshAvailability(env = process.env) {
...
export function windowsPowerShellAvailability(platform = process.platform, env = process.env) {
    if (platform !== 'win32')
        return { available: false };
    const sysRoot = String(env.SystemRoot ?? env.windir ?? 'C:\\Windows');
...
export function detectAvailability(platform = process.platform, env = process.env) {
    return {
 ...
 */
function probePowerShellVersion(executable) {
...
async function probePowerShellVersions(entries) {
    const found = new Map();
    await Promise.all(entries.map(async (entry) => {
        const version = await probePowerShellVersion(entry.path);
        if (version !== undefined)
            found.set(entry.kind, version);
    }));
    return found;
}
/**
 ...
 */
export async function detect(platform = process.platform, env = process.env) {
    ...
    const versions = await probePowerShellVersions([
        ...(pwsh.available && pwsh.path !== undefined ? [{ kind: 'pwsh', path: pwsh.path }] : []),
        ...(powershell.available && powershell.path !== undefined ? [{ kind: 'powershell', path: powershell.path }] : []),
    ]);
    const result = [];
    if (bash.available) {
        result.push({
...
export function detectSync(platform = process.platform, env = process.env) {
    const bash = bashAvailability(platform, env);
    const pwsh = pwshAvailability(env);
    const powershell = windowsPowerShellAvailability(platform, env);
    const result = [];
    ...
    if (pwsh.available)
        result.push({ kind: 'pwsh', name: 'PowerShell 7', ...(pwsh.path === undefined ? {} : { path: pwsh.path }) });
    if (powershell.available) {
        result.push({
            kind: 'powershell',
            name: 'Windows PowerShell',
